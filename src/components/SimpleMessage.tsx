...
export const SimpleMessage: React.FC<SimpleMessageProps> = ({ message }) => {
  ...
  const [feedback, setFeedback] = useState<"positive" | "negative" | null>(message.metadata?.feedback || null);
  const [isSubmittingFeedback, setIsSubmittingFeedback] = useState(false);

  const getUserInitials = () => {
    if (!user) return "U";
    const displayName = user.user_metadata?.full_name || user.user_metadata?.name || user.name || "Usuario";
    return displayName
      .split(" ")
      .map((n: string) => n[0])
      .join("")
  ...
  const handleFeedback = async (rating: "positive" | "negative") => {
    ...
    try {
      ...
      const updatedMessages = currentConversation.messages.map((msg) => (msg.id === message.id ? updatedMessage : msg));
      ...
      const response = await fetch(
        `${ENV.AI_API_BASE_URL}/api/conversations/${currentConversation.id}?user_id=${encodeURIComponent(user.email)}`,
        {
          method: "PUT",
          headers,
        ...
        <div className="flex flex-col">
          ...
          >
            ...
            {message.chart && (
              <div className="mt-3 p-3 bg-gray-50 rounded-xl">
                ...
                <div className="text-sm text-gray-700">{JSON.stringify(message.chart, null, 2)}</div>
              </div>
            )}
          ...
          {!isUser && (
            <div className="flex items-center justify-start mt-2 space-x-2">
              ...
              {isSubmittingFeedback && <span className="text-xs text-gray-500">Enviando...</span>}
            </div>
          )}
        </div>
      </div>
