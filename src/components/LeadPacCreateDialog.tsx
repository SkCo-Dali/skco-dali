...
export interface LeadPacCreateDialogRef {
  openDialog: () => void;
}

interface LeadPacCreateDialogProps {
  onLeadCreated?: () => void;
}

export const LeadPacCreateDialog = forwardRef<
  LeadPacCreateDialogRef,
  LeadPacCreateDialogProps
>((props, ref) => {
  ...
  const [formData, setFormData] = useState({
    ...
    numeroEmpleados: "",
    sectorEmpresa: "",
    estadoLeadPac: "Prospecto",
    notas: "",
  });

  const preCheck = useLeadPacPreCheck({
    userId: user?.id || "",
    autoVerify: false, // Verificación manual y onBlur
  });
  ...
  const handleContinueToStep2 = () => {
    ...
    if (preCheck.verificationResult?.exists && preCheck.verificationResult.tipoLead === "pac") {
      // Bloquear si ya existe un PAC
      return;
    }
  ...
  const handleOpenExistingLead = async () => {
    if (preCheck.verificationResult?.leadId) {
      ...
      // TODO: Implementar navegación al detalle del lead
      handleClose();
    }
  };

  const handleConvertToPac = async () => {
    if (preCheck.verificationResult?.leadId) {
      await preCheck.logDecision("convert");
      toast({
        title: "Conversión a PAC",
        description: "Funcionalidad de conversión en desarrollo",
      });
      // TODO: Implementar conversión de lead genérico/corporativo a PAC
      handleClose();
    }
  };

  const handleCreatePac = async () => {
    ...
    try {
      await createNewLead({
        ...
        estadoLeadPac: formData.estadoLeadPac,
      } as any);

      await preCheck.logDecision("create");

      toast({
        title: "Lead PAC creado",
        description: `Se ha creado exitosamente el lead PAC para ${formData.empresa}`,
      });

      handleClose();
      onLeadCreated?.();
    } catch (error) {
      console.error("Error creating PAC lead:", error);
      toast({
        title: "Error",
        description: "No se pudo crear el lead PAC. Por favor intente nuevamente.",
        variant: "destructive",
      });
    } finally {
      setIsCreating(false);
    ...
    <div className="space-y-4">
      ...
      {preCheck.hasVerified && preCheck.verificationResult && (
        <div className="mt-4 space-y-3">
          {/* Caso A: Coincidencia PAC por NIT */}
          {preCheck.verificationResult.exists &&
            preCheck.verificationResult.tipoLead === "pac" && (
              <Alert variant="destructive" className="border-destructive">
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  <p className="font-semibold mb-2">
                    Este NIT ya tiene un PAC registrado
                  </p>

                  {preCheck.verificationResult.inScope ? (
                    // Lead en alcance del usuario
                    ...
                    // Lead fuera de alcance - data minimization
                    <div className="space-y-2 text-sm">
                      <p>
                        Existe una oportunidad PAC registrada para este NIT.
                      </p>
                      <div>
                        <strong>Estado:</strong>{" "}
                        {preCheck.verificationResult.estado}
          ...
          {/* Caso B: Sin PAC pero hay lead genérico/corporativo */}
          {preCheck.verificationResult.exists &&
            preCheck.verificationResult.tipoLead !== "pac" && (
              <Alert className="border-primary">
                ...
                <AlertDescription>
                  <p className="font-semibold mb-2">
                    Existe un lead {preCheck.verificationResult.tipoLead}{" "}
                    asociado a este NIT
                  </p>
                  <div className="space-y-2 text-sm">
                    {preCheck.verificationResult.inScope && (
            ...
            <Alert className="border-success bg-success/5">
              ...
              <AlertDescription>
                ...
                <p className="text-sm mt-1">
                  Puedes continuar con la creación del lead PAC
                </p>
              </AlertDescription>
            </Alert>
          )}
  ...
  const renderStep2 = () => (
    <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
      ...
      <div className="space-y-2">
        <Label htmlFor="estadoLeadPac">Estado del Lead PAC</Label>
        <Select
          value={formData.estadoLeadPac}
          onValueChange={(value) =>
            setFormData({ ...formData, estadoLeadPac: value })
          }
        >
          <SelectTrigger>
            <SelectValue />
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Información adicional sobre el PAC"
          rows={3}
        />
      </div>
    </div>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>
            {currentStep === 1 ? "Verificación de NIT" : "Crear Lead PAC"}
          </DialogTitle>
          <DialogDescription>
            {currentStep === 1
              ? "Paso 1: Verifica que el NIT no esté registrado previamente"
              : "Paso 2: Completa la información del lead PAC"}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex justify-between mt-4">
          ...
          {currentStep === 1 ? (
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleCreatePac} disabled={isCreating}>
                {isCreating ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    Creando...
...
LeadPacCreateDialog.displayName = "LeadPacCreateDialog";
