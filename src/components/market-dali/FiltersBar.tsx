...
import { Search, SlidersHorizontal, X, Heart, ChevronDown, ChevronUp, RotateCcw } from "lucide-react";
...
export const FiltersBar: React.FC<FiltersBarProps> = ({ filters, onFiltersChange, onResetFilters }) => {
  const [isExpanded, setIsExpanded] = useState(false);

  const categories = Object.entries(CATEGORY_CONFIG) as [
    OpportunityCategory,
    (typeof CATEGORY_CONFIG)[OpportunityCategory],
  ][];
      ...
      <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
        <CollapsibleContent>
          <div className="p-4 bg-muted/50 rounded-lg space-y-4">
            {/* Categories */}
            <div>
              <label className="text-sm font-medium text-foreground mb-2 block">Categorías</label>
                ...
                {priorities.map(([key, config]) => (
                  <Badge
                    ...
                    className={cn(
                      ...
                      filters.priorities.includes(key) ? "" : config.color,
                    )}
        ...
        <div className="flex flex-wrap gap-2">
          {filters.categories.map((cat) => (
            <Badge key={cat} variant="secondary" className="gap-1 pr-1">
              ...
              <button onClick={() => toggleCategory(cat)} className="ml-1 p-0.5 hover:bg-muted rounded-full">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ))}
          {filters.priorities.map((prio) => (
            <Badge key={prio} variant="secondary" className="gap-1 pr-1">
              {PRIORITY_CONFIG[prio].label}
              <button onClick={() => togglePriority(prio)} className="ml-1 p-0.5 hover:bg-muted rounded-full">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ))}
