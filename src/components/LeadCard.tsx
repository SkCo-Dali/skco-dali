...
export function LeadCard({ 
  ...
  return (
    <Card 
      className="cursor-pointer hover:shadow-md transition-all duration-200 w-full border border-gray-200 bg-white relative"
      onClick={handleCardClick}
    >
      {/* Muesca con estado en la parte superior */}
      <div className="absolute -top-2 left-4 z-10">
        <div className="relative">
          <div className="w-3 h-3 bg-white border border-gray-200 rotate-45 transform"></div>
          <Badge 
            className={`absolute -top-1 -left-8 text-xs px-2 py-1 whitespace-nowrap ${stageColors[lead.stage as keyof typeof stageColors] || 'bg-gray-100 text-gray-800'}`}
            variant="secondary"
          >
            {lead.stage}
          </Badge>
        ...
        <div className="flex items-start justify-between">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild data-dropdown-trigger>
              <button className="h-4 w-4 text-gray-400 flex-shrink-0 hover:text-gray-600">
            ...
            <DropdownMenuContent align="end" className="w-48 bg-white border border-gray-200 shadow-lg">
              ...
              {onSendWhatsApp && (
                <DropdownMenuItem 
                  onClick={(e) => handleMenuClick(e, () => onSendWhatsApp(lead))}
                  className="flex items-center gap-2 px-3 py-2 hover:bg-gray-50 cursor-pointer"
                >
