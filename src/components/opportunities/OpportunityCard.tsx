      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            {/* Inactive alert */}
            {!opportunity.isActive && (
              <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/20 rounded-lg p-2 md:p-4">
                <Alert className="bg-blue-50 border-blue-200 w-full max-h-full overflow-y-auto">
                  <CheckCircle className="h-3 w-3 md:h-4 md:w-4 text-blue-600 shrink-0" />
                  <AlertDescription className="text-blue-900">
                    <p className="font-semibold text-xs md:text-sm mb-0.5 md:mb-1">✓ Clientes ya cargados</p>
                    <p className="text-[10px] md:text-xs mb-0.5 md:mb-1">Búscalos filtrando la Campaña:</p>
                    <p className="text-[10px] md:text-xs font-medium break-words">{opportunity.lastCampaignName || opportunity.title}</p>
                  </AlertDescription>
                </Alert>
              </div>
            )}
